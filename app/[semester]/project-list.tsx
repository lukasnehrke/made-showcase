...
function LoadMoreButton() {
  ...
  return (
    <Button className="text-sky-500 hover:text-sky-400" type="submit" variant="ghost">
      Load more projects
      {pending ? <Loader2 className="ml-2 w-4 h-4 animate-spin" /> : <ChevronsDown className="ml-2 w-4 h-4" />}
    </Button>
  );
}
    ...
    <div className="mt-8">
      {projects.length > 0 ? (
        <>
          ...
          {loadMore ? (
            <form action={formAction} className="flex items-center justify-center mt-8">
              <input name="semester" type="hidden" value={semester} />
              <input name="query" type="hidden" value={searchParams.get('query') ?? ''} />
